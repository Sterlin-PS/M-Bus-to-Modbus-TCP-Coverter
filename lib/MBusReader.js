...
class MBusReader {
  ...
  async readDevice(address) {
    ...
    try {
      ...
      // Wait for device to be ready (important for some meters)
      await this.delay(100);

      console.log(`  Sending REQ_UD2...`);
      const response = await this.sendReceive(this.buildReqUd2(address), 5000); // Longer timeout
  ...
  async readDeviceSecondary(secondaryAddr) {
    ...
    try {
      ...
      const response = await this.sendReceive(this.buildReqUd2(0xFD), 5000);
